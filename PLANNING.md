# Voice AI Agent Platform — Implementation Plan

This document breaks the full build into 10 numbered phases, ordered by dependency and risk.
...
**Acceptance Criteria:**
- All services can import from `shared.models` and `shared.utils`
- Database base supports multi-tenancy (tenant_id on all tables)
- Logging produces JSON output for production
- Custom exceptions follow FastAPI conventions
...
**Acceptance Criteria:**
- Tenant creation auto-generates secure API key (bcrypt hashed in DB)
- API keys can be rotated and revoked
- All endpoints require auth (JWT or API key)
...
- All requests resolved to correct backend service (tenant-service:8005, etc.)
- JWT validation extracts tenant_id from claims
- Rate limiting tracks requests per tenant_id + client IP
...
- HTML cleaning removes non-content elements
- Text chunks are 150-300 words with 10-word overlap
- Chunk metadata includes URL, index, word count
...
**Acceptance Criteria:**
- `/ingest` receives chunks and stores them with embeddings
- `/retrieve` returns top-k relevant chunks for a query
- `/context` formats chunks for LLM prompt injection
- All operations respect tenant isolation

**Estimated Complexity:** High (depends on PocketFlow integration)
...
**Acceptance Criteria:**
- `/chat` returns SSE stream of LLM responses
- System prompt includes guardrail: "Answer only using provided context"
- Input sanitization strips prompt injection patterns
- Model selection per tenant with fallback to default
...
**Acceptance Criteria:**
- `/token` returns LiveKit access token for WebRTC connection
- `/transcribe` accepts audio bytes and returns transcript
- `/synthesize` accepts text and returns audio bytes (WAV/PCM)
- STT model configurable (base | small | medium | large-v3)

**Estimated Complexity:** High (audio processing, WebRTC)
...
**Acceptance Criteria:**
- Widget embeds via `<script src="agent.js" data-tenant-id="..."></script>`
- Voice mode: mic → STT → LLM → TTS → speaker
- Chat mode: text input → SSE stream → text output
- Cross-origin requests to gateway (port 8000)
...
- Services communicate on `voiceai-net` internal network
- PostgreSQL, Redis, LiveKit, Ollama all configured
- External ports: 8000 (gateway), 7880 (LiveKit), 7881 (LiveKit Web), 8006 (widget)
...
**Acceptance Criteria:**
- All tests pass with `pytest services/ -v`
- Coverage targets: >80% for core services
- Integration tests use test containers (PostgreSQL, Redis)
...
|-------|-----------|------------|
| 1 | Shared Foundation | Low |
| 2 | Tenant Service | Medium |
| 3 | Gateway Service | Medium |
| 4 | Crawler Service | Medium |
| 5 | Knowledge Service | High |
| 6 | LLM Service | Low-Medium |
| 7 | Voice Service | High |
| 8 | Web Widget | Medium |
| 9 | Docker Composition | Medium |
